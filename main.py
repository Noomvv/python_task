...
def remind_message(message):
    ...
    bot.register_next_step_handler(message, get_remind_text, reminder_text=reminder_text)
...
def get_remind_time(message):
    sql.execute('INSERT INTO reminders (user_id, reminder, time, status) VALUES (?, ?, ?, ?)', (message.from_user.id, message.text, message.text, 'active'))
    connection.commit()
    connection.close()
